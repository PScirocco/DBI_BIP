...
4. まずシーケンスを保存（メニュー「シーケンスに名前を付けて保存…」）。
   実行中に「中断」→「再開」→「停止」→ 保存ダイアログ「はい」
   → `dist\BI-sim\bisim_output\_stops\<シーケンス名_NN_レシピ名>\` に
...
3. §6 の 1〜6 を同様に実施
4. 一度終了 → 再度起動 → メニュー「シーケンスを開く」で §6-4 で保存した
   シーケンス JSON を開く → ④実行タブで対象ステップを選び
   「▶↻ 停止位置から再開」が有効になり、続きから再開できること
